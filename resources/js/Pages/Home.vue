...
onMounted(() => {
    Window.addEventListenner('resize', () => {
...
<template>
    ...
    <MainLayout>
        <div class="mx-auto lg:pl-0 md:pl-[80px] pl-0">
            ...
            <div id="Posts" class="px-4 max-w-[600px] mx-auto mt-10" v-for="post in posts.data" :key="post">
                ...
                <!-- Render image if it is an image file -->
                <img v-if="isImage(post.file)" class="mx-auto w-full" :src="post.file" alt="">
                <!-- Render video if it is a video file -->
                <video v-else class="mx-auto w-full" :src="post.file" controls autoplay loop muted></video>
                </div>
                ...
                <button @click="$event => { currentPost = post; openOverlay = true }"
    ...
    <ShowPostOverlay v-if="openOverlay" :post="currentPost" @addComment="$event => addComment($event)"
        @updateLike="$event => updateLike($event)" @deleteSelected="$event =>
            {deleteFunc($event)
            openOverlay = false}
